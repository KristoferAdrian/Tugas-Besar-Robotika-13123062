function [robot_state, robot_params, pid_params, env_params, sim_params, path, phi, mask, seen_obstacles, custom_map] = config()
    ...
    [Xobs, Yobs] = meshgrid(0:30:210, 40:30:160);
    obstacles = [Xobs(:), Yobs(:)];
    ...
    robot_params.motor_inertia = 0.01; % Optional for more detailed wheel dynamics
    ...
    env_params.dt = 0.5;  % Add dt here if you want global
    ...
    % Initialize path and HPF arrays
    path = [robot_pos];

    phi = 0.5 * ones(grid_size);
    mask = ones(grid_size);
    seen_obstacles = zeros(0, 2);  % Initialize as empty Nx2 matrix

    % Initialize color map (you can replace turbo with any colormap)
    n_colors = 256;
