...
% Now you can proceed with your main loop code...

% Initialize local variables for APF+Bug
mode = 'apf';
bug_state = 'approach';
follow_side = 'left';
position_history = [];
...
while norm([robot_state.x, robot_state.y] - env_params.goal) > env_params.goal_threshold && time < sim_params.max_time
    % Update HPF based on proximity sensing
    ...
    % Update visualization if HPF changed
    if newly_detected
        set(h_phi, 'CData', phi);
        drawnow;
    end

    % Compute APF+Bug navigation direction
    ...
    % PID velocity control and dynamics update
    ...
    % Compute goal error (Euclidean distance)
    e_goal = norm([robot_state.x, robot_state.y] - env_params.goal);
    
    % Estimate desired values again for logging
